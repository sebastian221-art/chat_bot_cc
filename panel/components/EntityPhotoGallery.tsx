// 📄 ARCHIVO: panel/components/EntityPhotoGallery.tsx
'use client'
import { useEffect, useRef, useState } from 'react'
import { getEntityPhotos, addEntityPhoto, deleteEntityPhoto, uploadImage, EntityPhoto } from '@/lib/api'
import { Upload, X, Loader2, ImageOff } from 'lucide-react'

// Etiquetas disponibles por tipo — debe coincidir con ENTITY_LABELS del backend
const LABELS_BY_TYPE: Record<string, { value: string; label: string }[]> = {
  event: [
  ...
  raffle: [
    { value: 'afiche', label: '🖼️ Afiche' },
    { value: 'premio', label: '🏆 Foto del premio' },
    { value: 'otra', label: '📷 Otra' },
  ],
  knowledge: [
...
export default function EntityPhotoGallery({ entityType, entityId, accent = 'indigo' }: Props) {
  const [photos, setPhotos] = useState<(EntityPhoto & { id: number })[]>([])
  const [loading, setLoading] = useState(false)
  const [uploading, setUploading] = useState(false)
  const labels = LABELS_BY_TYPE[entityType] || LABELS_BY_TYPE.knowledge
  const [newLabel, setNewLabel] = useState(labels[0].value)
  ...
  const load = async () => {
    ...
    setLoading(true)
    try {
      const data = await getEntityPhotos(entityType, entityId)
      setPhotos(data)
    } catch {
      setPhotos([])
    } finally {
    ...
    return (
      <div>
        ...
        <p className="text-zinc-600 text-xs bg-zinc-800/50 border border-dashed border-zinc-700 rounded-xl px-3 py-4 text-center">
          Guarda primero — después podrás agregarle fotos aquí mismo
        </p>
      </div>
    )
  }
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 25 * 1024 * 1024) { alert('Máximo 25 MB por archivo'); return }

    setUploading(true)
    try {
      const url = await uploadImage(file)
      await addEntityPhoto(entityType, entityId, { photo_url: url, label: newLabel })
      await load()
    } catch (err: any) {
  ...
  const handleDelete = async (photoId: number) => {
    ...
    try {
      await deleteEntityPhoto(entityType, entityId, photoId)
      await load()
    } catch (err: any) {
